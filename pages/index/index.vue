			...
			<view class="content_box" style="margin-top: -4rpx;overflow: hidden;">
				<scroll-view class="scroll-box" scroll-y scroll-with-animation enable-back-to-top>
					<block v-if="template" v-for="(item, index) in template" :key="index">
						...
						<!-- 推荐商品 -->
						<qin-goods v-if="item.type === 'goods-list'" :detail="item.content" />
						<sh-hot-goods v-if="item.type === 'goods-group'" :detail="item.content"></sh-hot-goods>
...
import goodsVue from '../app/commission/goods.vue';
import QinGoods from './components/qin-goods.vue';
